...
> 給下一個開發 session 的快速接手筆記。最後更新:**2026-09-02**(例行開發 session,Playwright UI 迴歸測試擴充)。
...
- 開發分支:**建議每輪從最新 `main` 開新的短命分支,合併後即刪**(舊慣用分支 `claude/wind-farm-game-dev-ik3kt7` 已列入清理清單);本輪(Playwright UI 迴歸測試)工作在 `claude/zen-mendel-ok7xkk`,合併後即刪。
...
### 立即可做(免後端)
- **Playwright UI 迴歸測試擴充** — ✅ 首批(2026-09-01,登入/訪客/教學跳過/調度中心彈窗 focus trap)與交易所/出海/維修畫面擴充(2026-09-02,見上,接單→交易所鍵盤採購→出航→登船→鍵盤完成診斷測驗+SOP→完工)皆已完成,共 **14** 項,CI `e2e` job 已覆蓋;後續可再挑其餘 11 個彈窗中 1–2 個代表性樣本,或涵蓋大型組件大修(#overhaul)/加班搶修(#rush)/審慎返港再規劃(#carry)等目前尚未走過的分支路徑。
...
- **新增彈窗/鍵盤互動流程時,比照 `test/e2e.mjs`(2026-09-01 新增、2026-09-02 擴充)補一筆 Playwright 斷言**,而非只在開發 session 內一次性截圖驗證後就丟棄——這樣行為才會被 CI 的 `e2e` job 持續把關,不必每次改動都重新手動走一遍。
