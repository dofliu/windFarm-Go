...
> 給下一個開發 session 的快速接手筆記。最後更新:**2026-09-02**(文件對齊 session)。
...
### 立即可做(免後端)
- **Playwright UI 迴歸測試擴充** — ✅ 首批已完成(2026-09-01,見上,`test/e2e.mjs` 覆蓋登入/訪客/教學跳過/調度中心彈窗 focus trap,共 7 項,CI 已接 `e2e` job);後續可比照擴充到交易所(`MarketScreen`)、維修畫面(`RepairScreen` 診斷測驗+SOP)等其他核心互動流程,以及其餘 11 個彈窗中再挑 1–2 個代表性樣本。
...
- **新增彈窗/鍵盤互動流程時,比照 `test/e2e.mjs`(2026-09-01 新增)補一筆 Playwright 斷言**,而非只在開發 session 內一次性截圖驗證後就丟棄——這樣行為才會被 CI 的 `e2e` job 持續把關,不必每次改動都重新手動走一遍。
