...
- **教師面板 CSV 匯出** — 在現有教師檢視面板上加「匯出 CSV」，方便登分與課後分析。（本 session 記錄的延伸候選）
- **每日/每週挑戰** — 在 15 項里程碑成就之上，加入週期性挑戰任務，強化長線動機。（本 session 記錄的延伸候選）
- **營運趨勢儀表板** — 把莉莉財報與 KPI 擴充為可回顧的時間序列圖（售電/OPEX/可用率/停機損失），作為教學覆盤工具。（本 session 記錄的延伸候選）
