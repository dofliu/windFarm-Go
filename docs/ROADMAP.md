...
- **呈現**：三模式背景（模擬/實境/漫畫）、60° 俯瞰、多場景登塔（機艙/塔架/輪轂/甲板）、Web Audio 音效音樂、中英雙語。
...
- **更多故障型錄與科別** — 擴充 `faults.ts`/`incidents.ts`（如葉片裂紋、海纜、結構螺栓、塔基監測），補齊結構/HSE 科別的對應故障，並串接圖鑑與課程週次。
- **教師後台與班級報表強化** — 在現有 Apps Script 排行榜上，增加依班級碼的**全班進度/可用率/SLA 達成**彙整與匯出（CSV），方便評量。
- **成就 / 任務系統** — 里程碑成就（零安全事件、SLA 全季達標、戰情室 100% 妥善…）與每日/每週挑戰，強化長線動機。
