...
> Lead with zh-TW; English summaries follow. Last reviewed: 2026-09-25。

**✅ 每機獨立健康度 / RUL 預測性維護 — Stage 1(資料骨架 + 純展示)**(2026-09-25 例行 session):延續前一輪(2026-09-24)完成的 [RUL_DESIGN.md](RUL_DESIGN.md) 設計草案,依其「建議下一輪從 Stage 1 開始實作」的排程完成 Stage 1。`src/state/game.ts` 的 `Turbine` 新增兩個選配欄位:`wear?`(累積劣化度 0-100)、`age?`(自建置以來累積運轉天數),向後相容舊存檔(讀取一律經 `wearOf()`/`ageOf()` 補預設 0,不需要專門的存檔遷移函式改寫 `fleet` 陣列本身)。每日推進(`advance()` 內的逐日迴圈)新增劣化累積:正常運轉每日 `WEAR_PER_DAY_BASE`(0.4),故障待修中額外加速 `WEAR_FAULT_EXTRA_PER_DAY`(1.0),皆夾 0-100 界。三個既有維護動作同步接上下修:戰情室單機維修工單完工時(`OPS_DISPATCH`→逐日推進解析)該機組 `wear` 下修 `WEAR_REPAIR_RELIEF`(30,不歸零,呼應「修復不等於整新如新」);全場預防性定檢(`OPS_INSPECT`,`kind:"__sweep__"`)完工時全機隊下修 `WEAR_INSPECT_RELIEF`(15);計畫性保養(`SCHEDULED_SERVICE`)完工時全機隊下修 `WEAR_SCHEDULED_SERVICE_RELIEF`(25)。新增 `wearRiskTier()` 依 `wear` 分四級(良好/觀察中/高風險/危急,門檻與 `RUL_DESIGN.md` 第 5 節一致)與 `WEAR_RISK_LABEL`/`WEAR_RISK_ICON`。UI 端:`FleetOpsModal.tsx`(風場戰情室)機組陣列每格左下角疊加風險徽章(🟡觀察中/🟠高風險/🔺危急,良好層級不顯示徽章以避免畫面雜訊),`title` tooltip 附加風險標籤文字,並於圖例補上三級徽章說明。**刻意保留設計草案的非目標邊界**:`faultTurbines`/戰情室逐日新故障選取**仍是均勻隨機**,不吃 `wear`(Stage 2 才接上加權),`fleetHealth` 全場純量語意不變、兩者不耦合;不顯示精確劣化數字(僅風險分級圖示,呼應「預測有不確定性」的教學意涵,也不需要在此階段處理 `diagLevel` 閘門)。`test/run.mjs` 新增一節「每機獨立劣化 / RUL 預測性維護 Stage 1」共 8 項斷言:`wearOf`/`ageOf` 預設值與向後相容、`wearRiskTier` 四級門檻邊界、每日累積(含故障加速)、0-100 夾界、定檢/計畫保養/維修完工下修驗證(不歸零);另在既有 fuzz 測試(3000 次隨機動作)的 `checkInvariants` 補上 `wear`/`age` 範圍不變式。**已驗證測試有效性**:刻意暫時拿掉每日劣化累積那行程式碼後重跑,新增的 3 項斷言(每日累積、故障加速、100 夾界)如預期精確失敗,其餘測試不受影響,證明非空跑;還原後 `npm test` = **175 全綠**(167→175,新增 8 項)、`typecheck`/`build` 乾淨、`npm run e2e`(38 項,不變)全過,確認 `FleetOpsModal.tsx` 改動未造成既有 focus trap/UI 迴歸。`npm run sim` 不需重跑(本階段刻意不影響故障挑選機率與經濟平衡)。**改動範圍**:`src/state/game.ts`/`src/ui/FleetOpsModal.tsx`(app 程式碼)+ `test/run.mjs`(測試)+ `docs/RUL_DESIGN.md`/`docs/GAME_DESIGN.md`/`docs/MANUAL.zh-TW.md`/本檔/`docs/HANDOFF.md`/`docs/TEST_REPORT.md`/`README.md`/`README.zh-TW.md`/`STATUS.yaml`(文件),`public/sw.js` 快取版本 **v17→v18**。**已知限制/下一步**:Stage 2(接上故障挑選權重,`faultTurbines`/逐日新故障改依 `wear` 加權抽樣,需 `npm run sim` 重新校正)、Stage 3(單機定檢動作 + `diagLevel` 真數值接軌)、Stage 4(部件級細分,選配)仍待後續輪次推進,詳見 [RUL_DESIGN.md](RUL_DESIGN.md) 第 8 節。
...
3. **P2・教學深化(依課堂數據擇一)**:每機獨立健康度/RUL 預測性維護(見 [RUL_DESIGN.md](RUL_DESIGN.md);✅ Stage 1「資料骨架 + 純展示」已完成,2026-09-25,下一輪可開始 Stage 2「接上故障挑選權重」)、Exam 進階版(教師發布+雲端報告)、內容編輯器(教師資料驅動新增故障)。
...
- **每機獨立健康度 / RUL 預測性維護** — ✅ 設計草案已完成（2026-09-24，詳見 [RUL_DESIGN.md](RUL_DESIGN.md)：資料模型、與既有機制整合點、分 4 階段實作計畫）；✅ **Stage 1（資料骨架 + 純展示）已完成（2026-09-25）**：`Turbine.wear`/`age` 欄位、每日累積(含故障加速)、定檢/計畫保養/維修完工下修、戰情室風險徽章展示,不動故障挑選機率與經濟平衡。下一輪建議接續 **Stage 2（接上故障挑選權重，`faultTurbines`/逐日新故障改依 `wear` 加權抽樣，需 `npm run sim` 重新校正）**。
