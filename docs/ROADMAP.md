...
> Lead with zh-TW; English summaries follow. Last reviewed: 2026-08-31.
...
4. **P2・無障礙延伸**:✅ 鍵盤操作走完工單循環已完成(2026-08-31);尚待色盲配色全面審查、對話/音效字幕、其餘管理類 modal 的 focus trap。
...
- **無障礙延伸（後續）** — ✅ 工單循環鍵盤操作已完成（見上）；尚待：其餘次要彈窗（教師/個人檔案/圖鑑等管理類 modal）的鍵盤覆蓋、modal 開啟時 focus 移入與 Esc 關閉（目前 modal 無 focus trap，鍵盤使用者開啟後 Tab 會落回背景頁面，屬已知限制）、更全面色盲配色審查、對話／音效字幕與旁白。
...
- **無障礙與音效字幕（部分已完成）** — ✅ 減少動態(`prefers-reduced-motion` + 手動開關)與海象/故障狀態圖示化雙重編碼已完成(見 [GAME_DESIGN.md](GAME_DESIGN.md) §17)；✅ 工單循環鍵盤操作已完成(2026-08-31,見上「建議下一步優先序」);尚待規劃：其餘管理類 modal 的鍵盤/focus trap 覆蓋、更全面的色盲友善配色審查、對話/音效字幕與旁白。
