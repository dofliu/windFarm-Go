...
> Lead with zh-TW; English summaries follow. Last reviewed: 2026-09-24。
...
3. **P2・教學深化(依課堂數據擇一)**:每機獨立健康度/RUL 預測性維護(設計草案已完成,見 [RUL_DESIGN.md](RUL_DESIGN.md),下一輪可開始 Stage 1 實作)、Exam 進階版(教師發布+雲端報告)、內容編輯器(教師資料驅動新增故障)。
...
- **每機獨立健康度 / RUL 預測性維護** — ✅ 設計草案已完成（2026-09-24，詳見 [RUL_DESIGN.md](RUL_DESIGN.md)：資料模型、與既有機制整合點、分 4 階段實作計畫）；下一輪建議從 Stage 1（`Turbine.wear`/`age` 資料骨架 + 純展示，不動平衡數字）開始實作。
